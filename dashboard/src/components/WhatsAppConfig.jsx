...
export function WhatsAppConfig({ clientId, agentId }) {
  ...
  async function loadConfig() {
    ...
    try {
      ...
      // Config GET failed — still try to check evolution status
  ...
  const checkStatus = useCallback(async () => {
    try {
      ...
      if (status.connected) {
        ...
        stopPolling()
        // Retry loading config if it wasn't loaded
        if (!config) {
          try {
            const data = await api.get(`/clients/${clientId}/agents/${agentId}/whatsapp`)
            if (data) {
  ...
  async function handleSaveSettings() {
    ...
    try {
      ...
      if (provider === 'gohighlevel') {
        payload.ghl_location_id = ghlForm.ghl_location_id || null
  ...
  async function handleDeleteConfig() {
    const ok = await confirm({
      ...
      message: 'Esto desactivara el canal WhatsApp para este agente.',
  ...
  const isConnected = step === STEPS.CONNECTED
  ...
  return (
    <Card className="space-y-4">
      ...
      {/* Provider tabs — solo si NO esta conectado */}
      {!isConnected && (
      ...
      {provider === 'evolution' && (
        <>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-text-muted">
                    {connectionInfo?.phone_number ? `+${connectionInfo.phone_number}` : 'Conectado'}
                  </p>
                </div>
                <button
                  type="button"
          ...
          {/* Not configured */}
          {(step === STEPS.NOT_CONFIGURED || step === STEPS.ERROR) && (
      ...
      {/* ── GoHighLevel ────────────────────────────────── */}
      {provider === 'gohighlevel' && !isConnected && (
      ...
      {/* ══════════════════════════════════════════════════
          CONTROL CENTER — Solo cuando esta conectado
          ══════════════════════════════════════════════════ */}
      {(isConnected || (provider === 'gohighlevel' && config)) && (
        <>
          {/* ── Pause/Resume — Siempre visible ──────────── */}
          {isConnected && (
            <button
              type="button"
              onClick={togglePause}
          ...
          {activeTab === 'settings' && (
            <div className="space-y-3">
              ...
              {config && provider === 'gohighlevel' && (
                <Button variant="danger" onClick={handleDeleteConfig} className="text-xs">
                  <Trash2 size={14} className="mr-1 inline" /> Eliminar configuracion
                </Button>
              )}
            </div>
          )}
